...
#     # Lọc đơn hàng trong range
#     orders_within_range=order_totals[(order_totals >= minValue) & (order_totals <=maxValue)]
#     # Danh sách đơn hàng không trùng nhau
#     unique_orders = df[df['OrderID'].isin(orders_within_range)]['OrderID'].drop_duplicates().tolist()
#
#     return unique_orders
# df = pd.read_csv('../dataset/SalesTransactions.csv')
# minValue = float(input("Enter min Value:"))
# maxValue = float(input("Enter max Value:"))
# result = find_orders_within_range(df,minValue,maxValue)
# print("List of orders within the range:",result) SAIIIIIIIIIIII
